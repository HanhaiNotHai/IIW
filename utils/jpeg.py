...
class JpegTest(nn.Module):
    ...
    def get_path(self):
        return (
            self.path + ''.join(random.sample(string.ascii_letters + string.digits, 16)) + ".jpg"
        )
...
class JpegBasic(nn.Module):
    ...
    def std_quantization(self, image_yuv_dct: Tensor, scale_factor, round_func=torch.round):
        ...
        luminance_quant_tbl = (
            ...
            .round()
            .to(image_yuv_dct.device)
            .clamp(min=1)
            .repeat(image_yuv_dct.shape[2] // 8, image_yuv_dct.shape[3] // 8)
        )

        chrominance_quant_tbl = (
            (
                torch.tensor(
                    [
    ...
    def std_reverse_quantization(self, q_image_yuv_dct: Tensor, scale_factor):
        ...
        luminance_quant_tbl = (
            ...
            .round()
            .to(q_image_yuv_dct.device)
            .clamp(min=1)
            .repeat(q_image_yuv_dct.shape[2] // 8, q_image_yuv_dct.shape[3] // 8)
        )
    ...
    def idct(self, image_dct: Tensor):
    ...
    def rgb2yuv(self, image_rgb):
        ...
        image_yuv[:, 1:2, :, :] = (
            -0.1687 * image_rgb[:, 0:1, :, :]
            - 0.3313 * image_rgb[:, 1:2, :, :]
            + 0.5 * image_rgb[:, 2:3, :, :]
        )
        image_yuv[:, 2:3, :, :] = (
            0.5 * image_rgb[:, 0:1, :, :]
    ...
    def idct_rgb(self, image_quantization, pad_width, pad_height):
        ...
        image_rgb = image_ret_padded[
            :, :, : image_ret_padded.shape[2] - pad_height, : image_ret_padded.shape[3] - pad_width
        ].clone()

        image_rgb = image_rgb * 2 / 255 - 1
        return clamp(image_rgb, -1, 1)

    def subsampling(self, image: Tensor, subsample):
        if subsample == 2:
            split_num = image.shape[2] // 8
